...
- **2026-09-07 — dependency bump + doc corrections.** `npm audit` 13 -> 0: lockfile bumps within range (react-router-dom 7.14.x -> 7.18.3, postcss, nanoid, picomatch, …) plus vite 5 -> **6.4.3** (one major, not the 8.x that `audit fix --force` proposes; plugin-react ^4 already supports vite ^6). The react-router bump changes the inlined bundle bytes but **every render hash stayed identical** — no re-grading was needed, only a bundle/CSS re-upload.
- **`pnpm-lock.yaml` deleted.** It was stale (March 2026), nothing installed from it, and it doubled every Dependabot alert. npm + `package-lock.json` is now the single source of truth. It also removes an install-time ambiguity for Vercel, which auto-detects the package manager from whichever lockfile it finds.
- **Radius claim corrected in `conventions.md`.** It said "square corners everywhere (radius 0px)" — carried over from the dark era. The redesign uses a tight-but-nonzero scale: `rounded` 2px, `rounded-lg` 3px, `rounded-xl` 4px. Worth noting the lesson: the class-name validation pass catches names that do not exist, but it does **not** catch a prose claim about a *value*. Spot-check numeric claims against `tailwind.config.js` too.
