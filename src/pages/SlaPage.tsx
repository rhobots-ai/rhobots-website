    ...
    <main>
      ...
      <section className="py-24 px-8 md:px-24 bg-background border-b border-outline max-w-4xl">
        <div className="space-y-12">
          <div>
            <h2 className="font-headline text-2xl font-semibold text-on-surface mb-6">Uptime Commitment</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-0 border border-outline mt-8">
              {[
                ...
                <div key={item.label} className="p-8 border-r border-b border-outline last:border-r-0 text-center">
                  <div className="font-headline text-3xl font-semibold text-primary-fixed mb-2">{item.metric}</div>
          ...
          <div>
            <h2 className="font-headline text-2xl font-semibold text-on-surface mb-6">Support Tiers</h2>
            <div className="space-y-4">
              {[
                ...
                <div key={tier.severity} className="border border-outline p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div>
                    <h3 className="font-headline text-lg font-semibold text-on-surface">{tier.severity}</h3>
                  ...
                  <div className="flex gap-8">
                    <div>
                      ...
                      <div className="font-headline text-sm font-semibold text-primary-fixed">{tier.response}</div>
                    </div>
                    <div>
                      <div className="font-mono text-[10px] tracking-widest text-on-surface opacity-50 uppercase">Resolution</div>
                      <div className="font-headline text-sm font-semibold text-on-surface">{tier.resolution}</div>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div>
            <h2 className="font-headline text-2xl font-semibold text-on-surface mb-6">Service Credits</h2>
            <div className="text-on-surface text-sm leading-relaxed opacity-70 space-y-4">
