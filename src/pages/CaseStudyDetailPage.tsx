...
export default function CaseStudyDetailPage() {
  ...
  return (
    <main>
      ...
      {/* Hero */}
      <section className="py-24 md:py-32 px-8 md:px-24 border-b border-outline relative overflow-hidden">
        <div className="relative z-10 max-w-4xl">
          ...
          <h1 className="font-headline text-3xl md:text-4xl font-semibold text-on-surface leading-[1.05] mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-px bg-outline-variant/20 border border-outline-variant/20">
          {study.metrics.map((m) => (
            <div key={m.label} className="bg-surface-container-high p-7 md:p-10">
              <span className="block font-headline text-3xl md:text-4xl font-semibold text-primary-fixed leading-tight">{m.value}</span>
              <p className="font-headline text-xs font-bold text-on-surface mt-4">{m.label}</p>
      ...
      <section className="px-8 md:px-24 py-16 md:py-24 bg-surface border-b border-outline">
        <div className="max-w-4xl border-l-4 border-primary-fixed pl-8 md:pl-12 py-2">
          <p className="font-headline text-2xl md:text-4xl font-semibold text-on-surface leading-tight mb-6">{study.pullQuote.heading}</p>
      ...
      <section className="px-8 md:px-24 py-16 md:py-24 bg-background border-b border-outline">
        ...
        >
          ...
          <div className="flex flex-wrap items-baseline gap-x-4 gap-y-1 mb-4">
            <span className="font-headline text-2xl md:text-3xl font-semibold text-on-surface group-hover:text-primary-fixed transition-colors">{study.poweredBy.product}</span>
      ...
      <section className="py-24 px-8 md:px-24 bg-surface border-b border-outline text-center">
        ...
        <div className="flex flex-col sm:flex-row gap-0 justify-center">
          ...
          <Link to={study.poweredBy.path} className="bg-surface border border-outline text-on-surface px-10 py-5 font-headline font-semibold text-sm hover:bg-surface-container transition-all text-center">
            Explore {study.poweredBy.product}
          </Link>
        </div>
      </section>
