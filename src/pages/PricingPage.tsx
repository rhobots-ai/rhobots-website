        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-0 border-t border-l border-outline">
          {tiers.map((tier) => (
            <div key={tier.name} className={`p-10 border-r border-b border-outline flex flex-col ${tier.highlight ? 'bg-surface' : ''}`}>
              ...
              <h3 className="font-headline text-2xl font-semibold text-on-surface mb-2">{tier.name}</h3>
              <div className="font-headline text-4xl font-semibold text-primary-fixed mb-4">{tier.price}</div>
              ...
              <Link
                to="/demo"
                className={`block text-center px-8 py-4 font-headline font-semibold text-sm transition-all ${
                  tier.highlight
